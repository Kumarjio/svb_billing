...
class billing extends mysql
{
	...
	{
		$sql = "select bl.discount,bl.discount_reason from bill as bl where bl.wastage='".$billno."'";
		$rst = $this->execute($sql);
	...
	{
		$sql = "select cs.id,cs.name,cs.phone,cs.address from wastage as bl,customers as cs where bl.customer_id=cs.id and bl.bill_no='".$billno."'";
		$rst = $this->execute($sql);
		$r = mysqli_fetch_array($rst[0]);
	...
	public function getOrders($billNo)
	{
		...
		$sql .= "select bl.id,bl.recieved_amount,bl.returned_amount from wastage as bl where bl.bill_no='".$billNo."' LIMIT 1;";
		$rst = $this->execute($sql);
		while($r = mysqli_fetch_array($rst[0]))
		{
	...
	public function processNewOrders()
	{
		...
		if($orderData)
		{
		$sql .= "INSERT INTO `wastage_order` (`bill_no`,`pid`, `product_id`, `product_name`, `qty`, `type`, `vat`, `unit_price`, `actual_amnt`, `recieved_amnt`, `discount`,`stock_reduction`, `ip`) VALUES ".implode(",",$tempSql).";";
		$sql .= "INSERT 
					INTO `wastage` 
				...
				UPDATE `wastage` SET `status`='".$this->mode."', `flow`=concat(`flow`,'".$this->mode."') WHERE  `id`=(select LAST_INSERT_ID()) LIMIT 1;
				UPDATE `shop_detail` SET `cur_was_no`='".(++$nxtBill)."';";
		$rst = $this->execute($sql);
...
<?php
	...
	public function processOldOrders()
	{
		...
		$nxtBill  = $billno;
		$actualAmnt = $billamnt = $tax_amount =  0.0;
		$sqlTmp = "select GROUP_CONCAT(od.id)'id' from wastage_order as od where od.bill_no='".$billno."' and od.`is`=1 group by od.bill_no";
		...
		foreach($this->oldOrders as $id=>$data)
		{
			...
			{
			$sql .= "UPDATE 
					`stock` as s,wastage_order as od
					 SET s.`available`=s.`available`+od.stock_reduction-".$stock."
					WHERE s.`id`=od.product_id and od.id='".$id."';";		
			}
			$sql .="UPDATE `wastage_order` SET `qty`=".$this->oldOrders[$id]['qty'].", `actual_amnt`=".$actualAmntTmp.", `recieved_amnt`=".$recievedTmp.", `discount`=".$this->oldOrders[$id]['discount'].",`stock_reduction`=".$stock." WHERE  `id`=".$id." LIMIT 1;";
		}
		...
		if($orderData)
		{
		$sql .= "INSERT INTO `wastage_order` (`bill_no`, `product_id`, `product_name`, `qty`, `type`, `vat`, `unit_price`, `actual_amnt`, `recieved_amnt`, `discount`,`stock_reduction`, `ip`) VALUES ".implode(",",$tempSql).";";
		}
		$sql .="UPDATE `wastage` SET `status`='".$this->mode."', `flow`=concat(`flow`,'".$this->mode."'),`customer_id`=".$this->customerId.",`actual_amount`=".$actualAmnt.",`bill_amount`=".$billamnt.",`round_off`='".$roundoff."' WHERE  `bill_no`='".$billno."' LIMIT 1;";
		if(count($remove)>0)
		{
			if(!$this->getBillingMode())
			{
			...
			{
				$sql .= "UPDATE `stock` SET `available`=`available`+(select od.stock_reduction from wastage_order as od where od.id=".$id.") WHERE  `id`=(select od.product_id from orders as od where od.id=".$id.") LIMIT 1;";
			}
			}
			$sql .="UPDATE `wastage_order` SET `is`=0 WHERE  `id` IN (".implode(',',$remove).");";	
		}
		$rst = $this->execute($sql);
		$this->loadCurrentBill();
	}
	...
	{
		$sql ="UPDATE `wastage` SET `status`='".$this->mode."', `flow`=concat(`flow`,'".$this->mode."'),`cancel`=1,cancel_reason='".$reason."' WHERE  `bill_no`='".$billno."' LIMIT 1;";
		$sql .="select od.product_id,od.stock_reduction from wastage_order as od where od.bill_no='".$billno."' and od.`is`=1;";
		$rst = $this->execute($sql);
		$sql = '';
		while($r = mysqli_fetch_array($rst[0]))
		{
    ...
    {
        $sql = "update wastage set `lock`=1,`trans_close`=1 where bill_no='".$bNo."';";
        $rst = $this->execute($sql);
		?>
        <center><div class="savemodeOption" style="width:50%">
        <div class="alert alert-block alert-success savemodeOptions"> <a class="close" data-dismiss="alert" href="#">×</a>
