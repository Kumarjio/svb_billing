...
?>
    <form action="" method="post">
      <div class="box">
        <div class="box-head"><h3>Bill Payment Removal</h3></div>
        <br>
        <?php
		if(isset($_POST['remove']))
		{
			$sql = "update bill_payment set `is`=0,`cancel`=1,`cancel_time`=NOW() where id=".$_POST['rowid']."";
			$mysqlwb->execute($sql);
			?>
...
if(isset($_REQUEST['billno']))
{
	...
	if(mysqli_num_rows($rst[0])>0)
	{
		...
		$sql = "select * from billing.customers as c where c.id=".$customer." limit 1;
				select py.id, date_format(py.date,'%d-%m-%Y')'date',py.recieved,py.returned,py.paid from bill_payment as py where py.bill_no='".$billno."' and py.`recieved`-py.`returned`!=0 and `is`=1 ";
		$rst = $mysqlwb->execute($sql);
        ...
        <table class='table table-striped table-bordered'>
		  ...
		  ?>
			  ...
			  while($p = mysqli_fetch_array($rst[1]))
			  {
                  ...
                  <td style="text-align:center"><?php echo $mysqlwb->date_format($p['date']) ?></td>
                  <td style="text-align:right"><?php echo $mysqlwb->currency($p['recieved']) ?></td>
                  <td style="text-align:right"><?php echo $mysqlwb->currency($p['returned']) ?></td>
                  <td style="text-align:right"><?php echo $mysqlwb->currency($p['recieved']-$p['returned']) ?></td>
                  <td>
                  <form action="#" method="post">
                  <input type="hidden" name="billno" value="<?php echo $billno ?>" >
                  <input type="hidden" name="rowid" value="<?php echo $p['id'] ?>" >
        ...
        <table class="table table-striped table-detail" style="width:400px; font-weight:bold">
									<tr>
										<th>Actual_amount </th>
										<td style="text-align:right"><?php echo $mysqlwb->currency($actual) ?></td>
									</tr>
                                    <tr>
										<th>Round Off </th>
										<td style="text-align:right"><?php echo $mysqlwb->currency($round)  ?></td>
									</tr>
									<tr>
										<th>Discount</th>
										<td style="text-align:right"><?php echo $mysqlwb->currency($dis)  ?></td>
									</tr>
                                    <tr>
                                    ...
                                    <tr style="font-size:18px">
										<th>Net Amount</th>
										<td style="text-align:right"><?php echo $mysqlwb->currency($bill_amount)  ?></td>
									</tr>
                                    <tr style="font-size:18px">
                                    	<th>Paid</th>
                                        <td style="text-align:right"><?php echo $mysqlwb->currency($p_amount); ?></td>
                                    </tr>
                                    <tr style="font-size:18px">
                                    	<th>Balance</th>
                                        <td style="text-align:right"><?php echo $mysqlwb->currency($bill_amount-$p_amount); ?></td>
                                    </tr>
								</table></center>
        <?php
	}
