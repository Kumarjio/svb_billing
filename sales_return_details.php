...
?>
    <form action="" method="post">
      <div class="box">
        <div class="box-head"><h3>Sales Return Details</h3></div>
        <br>
...
if(isset($_REQUEST['billno']))
{
	...
	if(mysqli_num_rows($rst[0])>0)
	{
        ...
        <table class='table table-striped dataTable table-bordered dataTable-tools'>
          ...
          <tfoot>
          <tr>
              <th style="text-align:right" colspan="4">Net</th>
        ...
        <table class='table table-striped dataTable table-bordered dataTable-tools'>
		  ...
		  $o_tax = 0;
		  $o_recieved= 0;
		  foreach($sale_return as $id=>$data)
		  {
		  ?>
            <tr>
              <td style="text-align:center"><?php echo ++$sno; ?></td>
			  <td style="text-align:center"><?php echo $data['date']; ?></td>
              <td style="text-align:center"><a href="product_view.php?id=<?php echo $data['id'] ?>"><?php echo $data['pid'] ?></a></td>
              <td><a href="product_view.php?id=<?php echo $data['id'] ?>"><?php echo $data['name'] ?></a></td>
              <td style="text-align:center"><?php 
					echo $data['qty']." ".$data['type'];
			  ?></td>
              <td style="text-align:right"><?php 
			  	echo $mysql->currency($data['unit_price']*$data['qty']); 
			  	$o_actual += $data['unit_price']*$data['qty']; ?></td>
              <td style="text-align:right"><?php 
			  	echo ($data['unit_price']*$data['qty'])-((($data['unit_price']*$data['qty'])/100)*$data['discount']); 
			  	$o_discount += ($data['unit_price']*$data['qty'])-((($data['unit_price']*$data['qty'])/100)*$data['discount']);  ?></td>
              <td style="text-align:right"><?php 
			  	echo $data['tax_amount']; 
			  	$o_tax += $data['tax_amount']; ?></td>
              <td style="text-align:right"><?php 
			  	echo $mysql->currency($data['net_amount']); 
			  	$o_recieved += $data['net_amount']; ?></td>
            </tr>
            <?php
		  }
		  ?>
          ...
          <tr>
              ...
              <th style="text-align:right"><?php echo $mysql->currency($o_actual) ?></th>
              <th style="text-align:right"><?php echo $mysql->currency($o_discount) ?></th>
			  <th style="text-align:right"><?php echo $mysql->currency($o_tax) ?></th>
              <th style="text-align:right"><?php echo $mysql->currency($o_recieved) ?></th>
            </tr>
          </tfoot>
        </table><?php
