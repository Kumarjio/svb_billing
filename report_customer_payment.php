...
if(isset($_POST['filter'])){
	...
	?>
      <table width="100%" cellspacing="5" cellpadding="5">
		...
		<th colspan="2" style="vertical-align:top">Date:<?php echo date('d-m-Y') ?></th>
          ...
          <tr style="height:50px;"><th colspan="7"><center>STATEMENT : <?php echo $mysql->date_format($_POST['from']) ?> TO <?php echo $mysql->date_format($_POST['to']) ?></center></th></tr>
        ...
        <tr>
                  ...
                  <td style="text-align:center"><?php echo $mysql->date_format($from); ?></td>
                  <td style="text-align:center">-</td>
                  <td style="text-align:center">Balance Till - <?php echo $mysql->date_format($acc_from); ?></td>
                  <td style="text-align:right"><?php echo $mysql->currency($particular[$dat]['credit'][$id]);  $cre +=  $pre_balance ?></td>
                  <td style="text-align:right"><?php echo $mysql->currency(0); $deb += 0 ?></td>
                  <td style="text-align:right"><?php echo $mysql->currency($pre_balance);
                  $credit = $pre_balance;
                   ?></td>
                </tr>
        <?php
        foreach($date as $dat1=>$dat)
        {
            ...
            ?>
                <tr>
                  ...
                  <td style="text-align:right"><?php echo $mysql->currency($particular[$dat]['debit'][$id]); $deb +=  $particular[$dat]['debit'][$id]?></td>
                  <td style="text-align:right"><?php echo $mysql->currency($credit+$particular[$dat]['credit'][$id]-$particular[$dat]['debit'][$id]);
      ...
      <tr  style="border-top:#000 dashed 1px;border-bottom:#000 dashed 1px;" >
            ...
            <th style="text-align:right"><?php echo $mysql->currency($deb) ?></th>
            <th style="text-align:right"><?php echo $mysql->currency($cre-$deb) ?></th>
          </tr>
      </table>
	...
	foreach($bills as $bill_no=>$amnt){
		if($amnt==0) continue;?>
 	   <tr><td style="text-align:center"><?php echo ++$sno11 ?></td><td><?php echo $bill_no ?></td><td style="text-align:right"><?php echo $mysql->currency($amnt) ?></td><?php
	   $total += $amnt;
	}
	?>
    <tfoot>
    <tr><th colspan="2">Total</th>
    <th><?php echo $mysql->currency($total) ?></th></tr>
    </tfoot>
    </table>
<?php
}
