...
if(isset($_POST['load']))
{
	$sql = "select cs.id,cs.name,cs.phone,cs.address,cs.discount from customers as cs where cs.id=".$_POST['id'];
	...
	if($_GET['doy'])
	{
       ...
       window.opener.setCustomer(<?php echo $r['id'] ?>,'<?php echo $r['name'] ?>','<?php echo $r['phone'] ?>','<?php echo $r['discount'] ?>');
        </script>
        <?php
	}
}
