...
class billing extends mysql
{
	...
	{
		$sql = "select bl.discount,bl.discount_reason from store_purchase as bl where bl.wastage='".$billno."'";
		$rst = $this->execute($sql);
	...
	{
		$sql = "select cs.id,cs.dealer_name,cs.dealer_phone,cs.company_address from store_purchase as bl,dealer as cs where bl.customer_id=cs.id and bl.bill_no='".$billno."'";
		$rst = $this->execute($sql);
		$r = mysqli_fetch_array($rst[0]);
		$this->customerData[] = $r['id'];
		$this->customerData[] = $r['dealer_name'];
		$this->customerData[] = $r['dealer_phone'];
		$this->customerData[] = $r['company_address'];
	}
	...
	public function getOrders($billNo)
	{
		$sql = "select 
				...
				where 
					...
					and 
					od.`is`=1;";
		$sql .= "select bl.id,bl.recieved_amount,bl.returned_amount from store_purchase as bl where bl.bill_no='".$billNo."' LIMIT 1;";
		$rst = $this->execute($sql);
		while($r = mysqli_fetch_array($rst[0]))
		{
	...
	public function processNewOrders()
	{
		...
		if($orderData)
		{
		$sql .= "INSERT INTO `store_purchase_order` (`bill_no`,`pid`, `product_id`, `product_name`, `qty`, `type`, `vat`, `unit_price`, `actual_amnt`, `recieved_amnt`, `discount`,`stock_reduction`, `ip`) VALUES ".implode(",",$tempSql).";";
		$sql .= "INSERT 
					INTO `store_purchase` 
				...
				UPDATE `store_purchase` SET `status`='".$this->mode."', `flow`=concat(`flow`,'".$this->mode."') WHERE  `id`=(select LAST_INSERT_ID()) LIMIT 1;
				UPDATE `shop_detail` SET `cur_sto_no`='".(++$nxtBill)."';";
		$rst = $this->execute($sql);
...
<?php
	...
	public function processOldOrders()
	{
		...
		$nxtBill  = $billno;
		$actualAmnt = $billamnt = $tax_amount =  0.0;
		$sqlTmp = "select GROUP_CONCAT(od.id)'id' from store_purchase_order as od where od.bill_no='".$billno."' and od.`is`=1 group by od.bill_no";
		...
		foreach($this->order as $id=>$id1)
		{
			...
			$billamnt += $recievedTmp;
			$stock = $this->order[$id]['qty']/$this->type[$this->order[$id]['type']][$this->productsList[$id]['type']];
			$tempSql[] = "('".$billno."', ".$id.", '".$this->order[$id]['name']."', ".$this->order[$id]['qty'].", '".$this->order[$id]['type']."', ".$this->order[$id]['vat'].", ".$this->order[$id]['unit_price'].", ".$actualAmntTmp." , ".$recievedTmp.", ".$this->order[$id]['discount'].",".$stock.", '1')";		
			$orderData =1;
		}
		foreach($this->oldOrders as $id=>$data)
		{
			...
			$sql .= "UPDATE 
					`store_stock` as s,store_purchase_order as od
					 SET s.`qty`=s.`qty`+od.stock_reduction
					WHERE s.`pid`=od.product_id and od.id='".$id."';";
			$sql .="UPDATE `store_purchase_order` SET `qty`=".$this->oldOrders[$id]['qty'].", `actual_amnt`=".$actualAmntTmp.", `recieved_amnt`=".$recievedTmp.", `discount`=".$this->oldOrders[$id]['discount'].",`stock_reduction`=".$stock." WHERE  `id`=".$id." LIMIT 1;";
			$sql .= "UPDATE 
					`store_stock` as s,store_purchase_order as od
					 SET s.`qty`=s.`qty`-od.stock_reduction
					WHERE s.`pid`=od.product_id and od.id='".$id."';";
		...
		if($orderData)
		{
		$sql .= "INSERT INTO `store_purchase_order` (`bill_no`, `product_id`, `product_name`, `qty`, `type`, `vat`, `unit_price`, `actual_amnt`, `recieved_amnt`, `discount`,`stock_reduction`, `ip`) VALUES ".implode(",",$tempSql).";";
		}
		$sql .="UPDATE `store_purchase` SET `status`='".$this->mode."', `flow`=concat(`flow`,'".$this->mode."'),`customer_id`=".$this->customerId.",`actual_amount`=".$actualAmnt.",`bill_amount`=".$billamnt.",`round_off`='".$roundoff."' WHERE  `bill_no`='".$billno."' LIMIT 1;";
		if(count($remove)>0)
		{
			foreach($remove as $id)
			{
				$sql .= "UPDATE `store_stock` SET `qty`=`qty`+(select od.stock_reduction from store_purchase_order as od where od.id=".$id.") WHERE  `id`=(select od.product_id from store_purchase_orders as od where od.id=".$id.") LIMIT 1;";
			}
			$sql .="UPDATE `store_purchase_order` SET `is`=0 WHERE  `id` IN (".implode(',',$remove).");";	
		}
		$rst = $this->execute($sql);
		$this->loadCurrentBill();
	}
	...
	{
		$sql ="UPDATE `store_purchase` SET `status`='".$this->mode."', `flow`=concat(`flow`,'".$this->mode."'),`cancel`=1,cancel_reason='".$reason."' WHERE  `bill_no`='".$billno."' LIMIT 1;";
		$sql .="select od.product_id,od.stock_reduction from store_purchase_order as od where od.bill_no='".$billno."' and od.`is`=1;";
		$rst = $this->execute($sql);
		$sql = '';
		while($r = mysqli_fetch_array($rst[0]))
		{
    ...
    {
        $sql  = "update store_purchase set `lock`=1,`trans_close`=1 where bill_no='".$bNo."';";
		$sql .= "INSERT INTO `store_stock` (`pur_id`,`pid`, `product_name`, `qty`, `type`)  
				(select 
					s.id,
					s.product_id,
					s.product_name,
