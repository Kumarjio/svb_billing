...
class billing extends mysql
{
	...
	public function init()
	{
		$sql = "select
					...
					sd.billing_src
				from 
					billing.shop_detail as sd
				;";
		$rst = $this->execute($sql);
		$rst = mysqli_fetch_array($rst[0]);
		$this->tax =  $rst['tax'];
		if(isset($_REQUEST['cus_tax']))
			$this->tax = $_REQUEST['cus_tax'];
...
<?php
		$sql = "select c.`group`,c.name,c.`using` from billing.conversion as c where c.`is`=1 and c.`using`=1 order by c.`group`;
				select c.`group`,c.name,c.val,c.`using` from billing.conversion as c where c.`is`=1 order by c.`group`,c.`grade`;";
		$rst = $this->execute($sql);
		$grade = array();
...
<?php
	...
	{
		$sql = "select cs.id,cs.name,cs.phone,cs.address,cs.tin_no,cs.cst_no from bill as bl,billing.customers as cs where bl.customer_id=cs.id and bl.bill_no='".$billno."'";
		$rst = $this->execute($sql);
		$r = mysqli_fetch_array($rst[0]);
	...
	public function processNewOrders()
	{
		...
		foreach($this->order as $id=>$id1)
		{
			...
			$recievedTmp = $actualAmntTmp-(($actualAmntTmp/100)*$this->order[$id]['discount']);
			...
			{
				$sql .= "UPDATE billing.`stock` SET `available`=`available`-".$stock." WHERE  `id`=".$id." LIMIT 1;";
				$this->productsList[$id]['available'] = $this->productsList[$id]['available']-$stock;
			}else{
				$sql .= "UPDATE billing.`product` SET `available`=`available`-".$stock." WHERE  `id`=".$this->order[$id]['id']." LIMIT 1;";
			}
...
<?php
		...
		if($orderData)
		{
			...
			$sql .= "INSERT INTO `bill_order` (`bill_no`,`kot`,`pid`, `product_id`, `product_name`, `qty`, `type`, `vat`, `unit_price`, `actual_amnt`, `recieved_amnt`, `discount`,`stock_reduction`, `ip`) VALUES ".implode(",",$tempSql).";";
...
<?php
	...
	public function processOldOrders()
	{
		...
		foreach($this->oldOrders as $id=>$data)
		{
			...
			$actualAmnt += $actualAmntTmp;
			$recievedTmp = $actualAmntTmp-(($actualAmntTmp/100)*$this->oldOrders[$id]['discount']);
			$tax_amount += $recievedTmp;
			$billamnt += $recievedTmp;
			...
			{
				$sql .= "UPDATE 
							billing.`stock` as s,bill_order as od
						 SET 
				...
				$sql .= "UPDATE 
							billing.`product` as s,bill_order as od
						 SET 
						 	s.`available`=(s.`available`+od.stock_reduction)-".$stock."
						 WHERE 
			...
			$sql .="UPDATE `bill_order` SET `qty`=".$this->oldOrders[$id]['qty'].", `actual_amnt`=".$actualAmntTmp.", `recieved_amnt`=".$recievedTmp.", `discount`=".$this->oldOrders[$id]['discount'].",`stock_reduction`=".$stock." WHERE  `id`=".$id." LIMIT 1;";
		...
		if(count($remove)>0)
		{
			...
			foreach($remove as $id)
			{
				...
				$sql .= "UPDATE billing.`stock` SET `available`=`available`+(select od.stock_reduction from bill_order as od where od.id=".$id." and od.is=1) WHERE  `id`=(select od.product_id from bill_order as od where od.id=".$id." and od.is=1) LIMIT 1;";
			}
			}else{
				foreach($remove as $id)
				{
	...
	public function cancelBill($billno,$reason)
	{
		...
		while($r = mysqli_fetch_array($rst[0]))
		{
			$sql .= "UPDATE billing.`stock` SET `available`=`available`+".$r['stock_reduction']." WHERE  `id`=".$r['product_id']." LIMIT 1;";
		}
		$rst = $this->execute($sql);
	...
	public function printBill($bill_no,$duplicate=0)
	{
        ...
        <script type="text/javascript">
		  function print_bill(bill_no,dup)
		  {
			...
			{
			var myWindow=window.open('bill_printing_wb.php?bill_no='+bill_no+'&dup='+dup+'','','width=1,height=1');
	...
	public function print_output($billno,$duplicate=0)
	{
        ...
        <div id="customer">
          ...
          <table style="float:right;">
            <tr>
              <td style="border:none;font-size:16px;">Quotation No.</td>
        ...
        <table id="items">
          ...
          <tr class="item-row">
            ...
            <td class="item-name" style="border-right:#000 1px solid; font-size:11px">&nbsp;<?php echo html_entity_decode($prd); ?>&nbsp;</td>
          ...
          <tr style="height:10px;">
            ...
            <td colspan="2" class="total-line" style="border-left:1px #000000 solid;border-right:#000 1px solid">VAT(<?php echo $tax ?>%)</td>
            <td class="total-value" style="border-right:#000 1px solid;"><?php 
