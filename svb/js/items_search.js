...
$(document).ready(function() {
    ...
    $(".itemName").keydown(function(e) { 
	  ...
	  if (keyCode == 9) { 
		e.preventDefault();
		autoGetItem($(this).attr("id"),$(this).val());
	  } 
	});        
});
function addItem()
		...
		else
		{
		  ...
		  for(i=0;i<products.length;i++)
		  {
			...
			if(prst != -1 || prst1 != -1 || prst2 !=-1)
			{
				id = searchEnd++;
				count++;
				rst += '<tr id="sr'+(id)+'" class="searchResult" onclick="selectItem(\''+(i)+'\',\''+srcId+'\',\''+type+'\')" style="border-bottom:#CCC 1px dashed; height:25px;"> <td width="20%">'+products[i].id+'</td><td width="80%">'+products[i].name+'</td><td width="20%"><b><del>&#2352;</del></b>&nbsp;'+products[i].amnt+'    	</td>        </tr>';
				if(count > 10)
				{
...
function autoGetItem(srcId,val)
{
	ok = 0;
	srcId = srcId.substr(4,srcId.length);
	if(val!="")
	{
		i=0;
		for(;i<products.length;i++)
		{
			...
			{
				$("#item"+srcId).val(products[i].name);
				$("#netAmnt"+srcId).val(products[i].amnt);
				$("#amnt"+srcId).val(products[i].amnt);
				$("#qty"+srcId).focus();
				$("#qty"+srcId).val('1');
				$("#qty"+srcId).select();
				mkTotal();
