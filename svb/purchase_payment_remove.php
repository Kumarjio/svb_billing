...
?>
    <form action="" method="post">
      <div class="box">
        <div class="box-head"><h3>Purchase Payment Removal</h3></div>
        <br>
        <?php
		if(isset($_POST['remove']))
		{
			$sql = "update purchase_payment set `is`=0,`cancel`=1,`cancel_time`=NOW() where id=".$_POST['rowid']."";
			$mysql->execute($sql);
			?>
...
if(isset($_REQUEST['billno']))
{
	...
	if(mysqli_num_rows($rst[0])>0)
	{
		...
		$sql = "select * from dealer as c where c.id=".$customer." limit 1;
				select py.id, date_format(py.date,'%d-%m-%Y')'date',py.recieved,py.returned,py.paid from purchase_payment as py where py.bill_no='".$billno."' and py.`recieved`-py.`returned`!=0 and `is`=1  ";
		$rst = $mysql->execute($sql);
