...
function billingSettings(noCalc)
{
	...
	$("#MitemName").bind("keyup",function(e) {
		...
		else
		{
			...
			rst ='<table border="0" cellspacing="5" cellpadding="3" width="100%" ><thead><th>PID</th><th></th><th>Name</th><th align="right">Price</th></thead>';
		  	i=0;
		  	sel = Array();
		 	for(;i<products.length;i++)
		  	{
				prst = ((products[i].pid).toLowerCase()).indexOf(val);
				if(prst != -1)
				{
					sel[i] = 1;
					id = searchLen++;
					count++;
					rst += '<tbody class="searchResult"  id="sr'+count+'">  <input type="hidden" class="searchResultId" value="'+products[i].id+'" />  <tr>      <td align="center">'+products[i].pid+'</td><td><img src="'+products[i].photo+'" width="30" ></td><td>'+products[i].name+'</td><td align="right">'+products[i].amnt+'</td>      </tr></tbody>';
		  		}
		   	}
		   	i=0;
		 	if(val != "")
		 	{
			  for(;i<products.length;i++)
			  {
				  if(sel[i] == 1)
				  	continue;
				prst = ((products[i].name).toLowerCase()).indexOf(val);
				if(prst != -1)
				{
					id = searchLen++;
					count++;
