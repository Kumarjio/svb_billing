...
{	
	...
	if(parseFloat($("#MitemQty").val()) > parseFloat($("#MoldStock").val())){
			alert("Entered value is less than stock available");
			$("#MitemQty").val($("#MoldStock").val());
	}
...
function calcItemNet(noCalc)
{
	...
	$(".itemId").each(function(index, element) {
		if($(this).val()!='')
		{
			...
			if(parseFloat($(".itemQty:eq("+index+")").val()) > parseFloat($(".oldStock:eq("+index+")").val())){
					alert("Entered value is less than stock available");
					$(".itemQty:eq("+index+")").val($(".oldStock:eq("+index+")").val());
			}
			newStock = parseFloat($(".oldStock:eq("+index+")").val())-parseFloat($(".itemQty:eq("+index+")").val());
