...
?>
    <div class="box" style="min-height:450px;">
      ...
      <table class="table table-striped table-bordered" style="width:250px; overflow:visible" align="center">
        <thead>
          <tr>
          	<td style="white-space:nowrap; vertical-align:top">Filter : </td>
            <td style=" vertical-align:top"><input type="text" name="from" class="datepick input-small" value="<?php echo $from ?>" ></td>
            <td style="padding-top:15px; vertical-align:top">To</td>
            <td style=" vertical-align:top"><input type="text" name="to" class="datepick input-small" value="<?php echo $to ?>" ></td>
            ...
            <option value="charges_amnt" <?php if(in_array('charges_amnt',$_POST['column'])) echo 'selected'; ?> >Charges Amount</option>
...
<div style="padding-left:15px;"><input type="radio" name="status" value="s" <?php if($_POST['status']=='s') echo 'checked' ?> >&nbsp;&nbsp;Pending<br>
                <input type="radio" name="status" value="p" <?php if($_POST['status']=='p') echo 'checked' ?> >&nbsp;&nbsp;Printed<br>
                <input type="radio" name="status" value="c" <?php if($_POST['status']=='c') echo 'checked' ?> >&nbsp;&nbsp;Cancelled<br>
                <input type="radio" name="status" value="l" <?php if($_POST['status']=='l') echo 'checked' ?> >&nbsp;&nbsp;Locked without printing<br>
                </div>
<br>
                </div>
            	</td>
    ...
    <?php
		if(isset($_POST['submit']))
		{
            ...
            <?php
			$sql = "SELECT `id`,`".implode('`,`',$_POST['column'])."` from bill where `is`=1 and date between '".$from."' and '".$to."' ";
			if(isset($_POST['lock']))
			{
				$sql.=' and `lock`=1 ';
			}
				...
				foreach($_POST['column'] as $c)
				{
					...
					switch($c)
					{
						...
						default:
							...
							{
								?><a href="bill_detail.php?billno=<?php echo $p[$c] ?>"><?php echo $p[$c]; ?></a><?php
							}
							else
							{
								echo $p[$c];
